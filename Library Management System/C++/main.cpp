...
bool login(int t=0){ clear();
    ...
    if (check(currentuser.id,8)){
        cout << "\n\t\t<!> you are listed in the blacklist\n";
        cont(1);
        return false;
    }
    ifstream file(files[t]);
...
void borrowBook(){ clear();
    ...
    if(check(value, 2)){
        if(!check(value, 7)){
            ...
            }open.close();write.close();
        }else
            cout << "\n\n\t\t<!> Sorry, someone Already borrowed the book! You can try another one." << endl;
    }else
        cout << "\n\n\t\t<!> Could not find the book. Check your Book ID!" << endl;
    cont();
}
// Books borrowed by current user
void myBooks(int t = 7){ char bid[50], bn[50], un[50]; int i = 0; clear();
    switch (t){
    case 6:
        cout << "\n\n\t\t------------------ My Requests -------------------\n"; break;
    case 7:
...
void myInfo(){ clear();
    ...
    cout << "\n\t\tType: " << currentuser.type << endl; cont();
}
        ...
        while (open >> bid >> bn >> an) {
            if (strcasecmp(bid, search) == 0) {
                ...
                switch(y){
                    ...
                    default:
                        cout << "\n\n\t\t<!> Process canceled!\n";
                        write << setw(10) << left << bid << "\t" << setw(40) << left << bn << "\t" << setw(20) << left << an << endl; break;
                }
            }else{
...
void acceptreturn(){ string bn, uid; char value[50], bid[50];
    ...
    if(check(value, 7)){
            ...
            ofstream neww("temp.txt",ios::out|ios::app);
            while (open >> bid >> bn >> uid) {
                if (strcasecmp(bid, value) == 0) {
                    string confirm; char y;
...
void addblacklist(){ string un, up; char value[50], uid[50], reason[100];
    cin.ignore(); cout << "\n\n\t\t>> Enter User ID to add to blacklist: ";
    cin.getline(value, 50);
    if(check(value, 0)){
            ifstream open(files[0]);
            ofstream neww(files[8],ios::out|ios::app);
